    ...
    always @ (posedge phy_tx_clk) begin
        if (phy_tx_ctl != ctl_last) begin
            if (ctl_last) begin // we were transmitting, so cycle_counter reflects the packet length in bytes
                assert (cycle_counter >= 67 && cycle_counter <= 1530) else $error("RGMII TX packet length out of bounds: %0d bytes", cycle_counter);
                // minimum packet is 56 payload + 4 CRC + 7 byte preamble (sometimes the SFD is counted as part of the preamble, sometimes not, so we will just say 7 bytes of preamble)
