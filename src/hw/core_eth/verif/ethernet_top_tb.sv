...
class test_base;
    ...
    task wait_for_idle();
        ...
        repeat(10) @(posedge tb_if.clk_125M_i); // wait a few cycles for the packet to be processed
    endtask

    task simulate_rx_packet(input logic [47:0] dest_mac, input logic [7:0] payload[]);
        automatic logic [7:0] preamble [0:7] = '{8'h55, 8'h55, 8'h55, 8'h55, 8'h55, 8'h55, 8'h55, 8'hD5};
    ...
    task reset_dut();
        ...
        repeat(5) @(posedge tb_if.clk_125M_i); // wait for reset to propagate
    endtask
...
class misc_test_cases extends test_base;
    ...
    task mac_filtering_test();
        ...
        repeat(3) @(posedge this.tb_if.clk_125M_i);
