...
class Tags():
    ...
    EDGE  = 40   # this tag covers everything else barring a small number of very long tests
    LONG  = 50   # very long tests
    # these tags should be used mutually exclusively
...
class TestConfiguration:
    ...
    # Only run tests that match this callable when called as `tag_matcher(test_tags: list[Tags])`
    # (uncomment one of these to choose a test set. For typical development use SMOKE, MAIN, or EDGE. Run LONG at least once before submitting a PR.)
    #tag_matcher = lambda tags:min(tags) <= Tags.SOLO       # only run tests with the SOLO tag, useful for debugging a single test
    #tag_matcher = lambda tags:min(tags) <= Tags.MICRO      # only run tests with the MICRO tag, useful for testing the testbench configuration
    #tag_matcher = lambda tags:min(tags) <= Tags.SMOKE      # only run smoke tests which cover the DUT's hot paths
    #tag_matcher = lambda tags:min(tags) <= Tags.MAIN       # main test bench covering all features but excluding edge cases
    #tag_matcher = lambda tags:min(tags) <= Tags.EDGE       # includes edge cases but excludes long-running tests
    #tag_matcher = lambda tags:min(tags) <= Tags.LONG       # includes long-running tests
    tag_matcher = lambda x:True # everything
...
def create_test(
    ...
    timeout_us = 200                        # timeout for this test in microseconds of sim time
):
    is_micro = Tags.MICRO in tags
    is_smoke = Tags.SMOKE in tags
    is_main  = Tags.MAIN  in tags
    is_edge  = Tags.EDGE  in tags
    is_long  = Tags.LONG  in tags
    if sum([is_micro,is_smoke,is_main,is_edge,is_long]) != 1:
        raise ValueError(f"Test has tags: {tags}. Tests should have exaclty one of the mutually-exclusive tags: MICRO, SMOKE, MAIN, EDGE, LONG")
    ...
    def decorator(func):
        if not TestConfiguration.tag_matcher(tags):
            ...
            async def wrapped_function(dut):
                ...
                # By re-seeding for each test, we ensure you can re-run a single test and get the same results, even if the tests are run in a random order
                random.seed(cocotb.RANDOM_SEED + hash(func.__name__))
                ...
                try:
                    await with_timeout(func(dut_wrapped),timeout_us,"us")
