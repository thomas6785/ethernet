...
async def back_to_back_rx_buffer_test(dut_wrapped):
    ...
    await dut_wrapped.read_packet(0) # this reads back-to-back
...
async def back_to_back_csr_test(dut_wrapped):
    # This is a cool one
    # we'll generate a bunch of random reads and writes to various CSRs
    # and then send them all to the Python AXI driver at once in a random order
    # the AXI driver should queue them up and execute them back-to-back
    # and the mirror will constantly check that the readbacks match the writes
    for _ in range(5):
        ...
        processes = [
            *[dut_wrapped.intr_mask_set(randint(0,127))                             for _ in range(3)],
            *[dut_wrapped.intr_mask_get()                                           for _ in range(3)],
            *[dut_wrapped.intr_state_get()                                          for _ in range(3)],
            *[dut_wrapped.status_get()                                              for _ in range(3)],
            *[dut_wrapped.mode_set(random.choice([0,1]),random.choice([0,1]))       for _ in range(3)],
            *[dut_wrapped.mode_get()                                                for _ in range(3)],
            *[dut_wrapped.mac_address_set([randint(0,255) for _ in range(6)])       for _ in range(3)],
        ]
        ...
        # the model will update as soon as each process completes on the DUT, so we can check the model at any time and they should stay in sync

        await dut_wrapped.status_get()
        await dut_wrapped.mac_address_get() # because the MAC address is split over two registers, writes are not atomic, so we can't include them in the above processes because the model treats it as one action
...
async def buffer_almost_full_edge_test(dut_wrapped):
    ...
    space_used      = 0
    first_pkt_len   = randint(1330,1517) # has to be less than 1518 so we can send a +1 packet later to push us over the threshold
    pkt_len         = first_pkt_len
    for _ in range(4):
        space_used += pkt_len
        await dut_wrapped.tx_packet_send(pkt_len)
        await dut_wrapped.wait_for_tx_done()
        pkt_len = randint(1330,1518)
    ...
    # now pop the first packet out and send a new one, slightly larger, that should push us over the threshold
    await dut_wrapped.rx_pop_packet()
    await dut_wrapped.status_get()
...
async def crc_error_test(dut_wrapped):
    # Prepare a random packet and begin receiving over RGMII
    # after a random amount of time while mid-packet, force the RGMII data to 0x0
    # then assert that no packet arrives in the RX buffer

    data = [random.randint(0,255) for _ in range(1518)]
    await dut_wrapped.begin_rx_packet(data)

    for _ in range(randint(9,1518+9+4)):
        # 9 is the lower bound since it takes 9 cycles to start the packet and do the preamble
        # add 8 and 4 to the upperbound for the preamble and the CRC
        await RisingEdge(dut_wrapped.dut.clk_125M_i)

    await Timer(1, unit="ns") # we have to set just after the posedge to avoid a race condition with the RGMII driver
    dut_wrapped.dut.eth_rgmii_rx_data.value = int(dut_wrapped.dut.eth_rgmii_rx_data.value)^(0x1<<randint(0,3)) # force a CRC error by toggling a random bit
    await RisingEdge(dut_wrapped.dut.clk_125M_i)

    await dut_wrapped.wait_for_rx_done() # wait for the RX to finish
    n_pkts = (await dut_wrapped.status_get())>>8
    assert n_pkts == 0, f"Expected 0 packets pending after sending a packet with a CRC error, got {n_pkts}"

# TODO that test is good for making sure the RX basically works but it could do with more
# for example checking the other flags and comparing against the model
# but the model currently doesn't have a way of injecting CRC errors
...
@create_test(
    ...
    promiscuous=True,
    health_test_after=True,
    skip=True # TODO remove this. Currently this test violates an assertion in the DUT which causes Verilator to stop, and all subsequent tests to fail
)
async def illegal_long_packet_test(dut_wrapped):
    ...
    await dut_wrapped.tx_packet_send(randint(1519,2047)) # send an illegal long packet
    await dut_wrapped.wait_for_tx_done() # wait for the TX to finish
...
async def tx_write_during_tx_recovery(dut_wrapped):
    ...
    while await dut_wrapped.tx_busy():
        ...
        # TODO open an issue: this test CAN fail IF you have assertions in the DUT, because tx_axis.data CAN change without tx_axis_tready being asserted if you happen to overwrite the data. This is incredibly unlikely to happen and doesn't happen at all if assertions aren't enabled, but it's still worth documenting
...
async def rgmii_rx_test(dut_wrapped):
    await dut_wrapped.simulate_rx_packet([randint(0,255) for _ in range(randint(61,1517))])
...
async def rgmii_tx_test(dut_wrapped):
    await dut_wrapped.tx_packet_send(randint(61,1517))
...
async def short_packet_padding_test(dut_wrapped):
    await dut_wrapped.tx_packet_send(randint(2,59)) # send a short packet
    # TODO add tests for packets of size 0 and 1 - they are illegal but the DUT will just try send them without padding which causes an assertion to fail
    await dut_wrapped.wait_for_tx_done() # wait for the TX to finish
...
async def long_random_test(dut_wrapped):
    # randomly pop and inject packets for 100 iterations
    # check the status every iteration
    # every 10 iterations, read the RX buffer metadata and see if all packets match the model
    for _ in range(10):
        for _ in range(10):
            status = await dut_wrapped.status_get()
            ...
            if not table_almost_full and not table_full and not buf_almost_full:
                ...
                # either table_almost_full or buf_almost_full is set, 75% chance of a pop
            ...
            # Randomly delay before the next iteration
...
async def err_write_rx_buffer(dut_wrapped):
    ...
    assert bresp.resp == 2 # device error expected after a write to read-only buffer
...
async def err_write_rx_metadata(dut_wrapped):
    ...
    assert bresp.resp == 2 # device error expected
...
async def err_read_tx_buffer(dut_wrapped):
    ...
    assert rresp.resp == 2 # device error expected
...
async def err_read_invalid_address(dut_wrapped):
    ...
    rresp = await dut_wrapped._axi_driver.read(address,8)
    assert rresp.resp >= 2 # device error or decode error expected
...
async def err_write_invalid_address(dut_wrapped):
    address = random.randint(0x6040,0x7FFF) # pick a random address in the unused space
    bresp = await dut_wrapped._axi_driver.write(address,random.randbytes(8))
    assert bresp.resp >= 2 # device error or decode error expected
...
async def err_write_status_reg(dut_wrapped):
    ...
    # Assert that the write gave an error
    bresp = await dut_wrapped._axi_driver.write(csr.STATUS,random.randbytes(4))
    assert bresp.resp == 2 # device error expected
