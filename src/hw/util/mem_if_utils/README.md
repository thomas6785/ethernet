...
The terms 'host' and 'device' are used in this document to refer to the component responsible for initiating transactions and the component who responds to those transactions, respectively.

| Name              | Width         | Driven by | Validity                                    | Note |
|-------------------|---------------|-----------|---------------------------------------------|------|
| `req.req`         | 1             | Host      | Always valid.                               | Request access to memory (either read or write). |
| `rsp.gnt`         | 1             | Device    | Always valid.                               | Grant access.                                    |
| `req.addr`        | `ADDR_W`      | Host      | Valid when `req.req`                        | Address to access.                               |
| `req.data`        | `DATA_W`      | Host      | Valid when `req.req && req.we`              | Write data.                                      |
| `req.we`          | 1             | Host      | Valid when `req.req`                        | Write enable.                                    |
| `req.be`          | `DATA_W/8`    | Host      | Valid when `req.req && req.we`              | Write byte enable.                               |
| `rsp.err`         | 1             | Device    | Valid AFTER `req.req && rsp.gnt`            | Error response (to read or write).               |
| `rsp.data`        | `DATA_W`      | Device    | Valid AFTER `req.req && rsp.gnt && !req.we` | Read data response (to a read).                  |

Some behaviours of note:
* The `rsp.err` and `rsp.data` signals are valid on the cycle AFTER the handshake. This allows devices who do not require wait states to tie `req.gnt = 1` and give single-cycle latency.
* `rsp.err` and `rsp.data` are only valid for a single cycle so the host must be able to sample them immediately.
* A device may assert `req.gnt` proactively or reactively. A host must drive `req.req` proactively to avoid deadlocks. Devices may (and often will) tie `req.gnt = 1`.
...
* If an error response is given, `rsp.data` may be undefined. However, memory infrastructure should still route the signal as if it it were valid, and devices MAY use this to give a response to the host e.g. indicating the nature of the error.
...
* Cycles 16-20 show back-to-back transactions with a wait state inserted by the device.
* Cycles 24-28 show how the device can delay a transaction for an arbitrary length of time.
* Cycles 34-36 show a device asserting `rsp.gnt` proactively.
* Cycles 39-42 show a device assertion `rsp.gnt` proactively for back-to-back transactions.
