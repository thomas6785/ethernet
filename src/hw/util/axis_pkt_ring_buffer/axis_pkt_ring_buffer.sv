...
module axis_pkt_ring_buffer #(
    ...
    //  - the buffer was full at any point during the packet (while tvalid was asserted)
    //  - the external abandon signal was asserted at any point during the packet (while tvalid was asserted)
    //  - the descriptor table was full AT THE COMPLETION of this packet (it's okay if it was full at the start)
    ...
    `ASSERT(EmptyAfterReset_A,                  rst_ni && !$past(rst_ni) |-> empty_o,                       clk_i, 0);
    `ASSERT(TableNotFullAfterReset_A,           rst_ni && !$past(rst_ni) |-> !table_full_o,                 clk_i, 0);
    `ASSERT(TableNotAlmostFullAfterReset_A,     rst_ni && !$past(rst_ni) |-> !table_almost_full_o,          clk_i, 0);
    `ASSERT(BufNotAlmostFullAfterReset_A,       rst_ni && !$past(rst_ni) |-> !buf_almost_full_o,            clk_i, 0);
    `ASSERT(NPktsZeroAfterReset_A,              rst_ni && !$past(rst_ni) |-> n_pkts_buffered_o == '0,       clk_i, 0);
    `ASSERT(PacketLostNotAfterReset_A,          rst_ni && !$past(rst_ni) |-> !packet_lost_o,                clk_i, 0);
    `ASSERT(ReadyAfterReset_A,                  rst_ni && !$past(rst_ni) |-> pkt_tready_o,                  clk_i, 0);
