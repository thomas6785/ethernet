...
uint8_t ethernet_tx_packet_trigger(ethernet_t ethernet, uint16_t len_bytes) {
    if ((len_bytes > 1518) || (len_bytes < 60)) { // 1518 is the max size without the CRC, 60 is the min size without the CRC
...
// Reads the oldest packet from the RX buffer and copies its data into the provided buffer
// Also pops the packet from the RX buffer, freeing up space for new packets
// Returns the metadata for the packet, including its length
...
uint8_t ethernet_send_packet(ethernet_t ethernet, uint8_t *data, uint16_t len_bytes) {
    ...
    if (len_bytes < 60) { // min size without the CRC
